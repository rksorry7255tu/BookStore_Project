...
const Cart = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      const response = await axios.get(
        "https://the-book-spot-by-ravi.onrender.com/api/v1/get-user-cart-book",
        { headers }
      );
  ...
  const deleteItem = async (bookid) => {
    const response = await axios.put(
      `https://the-book-spot-by-ravi.onrender.com/api/v1/delete-From-Cart/${bookid}`,
      {},
      { headers }
    );
  ...
  const PlaceOrder = async () => {
    try {
      const response = await axios.post(
        "https://the-book-spot-by-ravi.onrender.com/api/v1/place-order",
        { order: Carts },
        { headers }
      );
      //console.log(response.data);
